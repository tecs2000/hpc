...
#pragma omp parallel for reduction(+ : error)
    ...
    cout << "Error rate: " << (float)error / testset.size() * 100 << "%" << endl;
...
float compute_euclidean_distance(const Entry &point, const Entry &entry) {
    ...
    for (size_t i = 0; i < point.features.size(); ++i) {
        float diff = point.features[i] - entry.features[i];
        sum += diff * diff;
    }
    return sum;
}
...
#pragma omp parallel for
    ...
    return max_element(voting.begin(), voting.end(), 
...
void process_cmdline(int argc, char *argv[]) {
    if (argc < 4 || argc > 5) {
        cout << "Usage: " << argv[0] << " <trainset-path> <testset-path>[optional] <preprocess>[1, 0] <k>" << std::endl;
        exit(1);
    } else {
